...
// Creates a completely new final object (with simpler but visually identical mesh) to replace the object used in the drawing phase.
public class MeshOptimizer : MonoBehaviour
{
    ...
    private short[,] terrainArray;
    private PropagatorManager propagatorManager;
    private HashSet<Vector2Int> uniquePoints; // hashset to keep track of unique points in the mesh so that duplicates aren't added.

    public static MeshOptimizer Instance { get; private set; }
    ...
    public void SpawnNewObject(short objectIndex)
    {
        ...
        int height = propagatorManager.PropagatorHeights[objectIndex];
        ...
        if (go != null)
        {
            ...
            go.transform.SetParent(ParentObject.transform);  // Just for editor grouping
            stopwatch.Stop();
            if (Configuration.DebugOutput)
            {
                Debug.Log($"Spawned object #{objectIndex} in {stopwatch.ElapsedMilliseconds} ms.");
            }
        }
    }
    ...
    {
        short[,] terrainArray = GetComponent<GameManager>().terrainGrid;
        GameObject go = new GameObject();
        go.name = objectNumber.ToString();
        MeshFilter mf = go.AddComponent<MeshFilter>();
        MeshRenderer mr = go.AddComponent<MeshRenderer>();
        mr.material = new Material(Shader.Find("Standard"));
        mr.material.color = propagatorColorManager.GetFinalColor(objectNumber);
        mr.shadowCastingMode = UnityEngine.Rendering.ShadowCastingMode.Off;
        mr.receiveShadows = false;

        List<Vector2Int> contourPoints = TraceContour(terrainArray, objectNumber);
//        Debug.Log($"{stopwatch.ElapsedMilliseconds} ms - traced contour - found {contourPoints.Count} points.");

        List<Vector2Int> optimizedContourPoints = OptimizeSideContourPoints(contourPoints);
//        Debug.Log($"{stopwatch.ElapsedMilliseconds} ms - optimized side contour into {optimizedContourPoints.Count} points.");

        Mesh topMesh = CreateHorizontalMeshFromContourPoints(optimizedContourPoints, height, false);
        Mesh bottomMesh = CreateHorizontalMeshFromContourPoints(optimizedContourPoints, 0, true);
 //       Debug.Log($"{stopwatch.ElapsedMilliseconds} ms - created top and bottom meshes from optimized contour points.");

        Mesh sideMesh = CreateSideMeshFromContourPoints(optimizedContourPoints, height);
 //       Debug.Log($"{stopwatch.ElapsedMilliseconds} ms - created side mesh from optimized contour points.");

        List<Mesh> meshestToCombine = new List<Mesh> { topMesh, bottomMesh, sideMesh };
        Mesh mesh = CombineMeshes(meshestToCombine);

        if (mesh.vertexCount < Configuration.MinimumObjectSize)
        {
            go.name += " (discarded small object)";
            go.transform.SetParent(ParentObject.transform);
            if (Configuration.DebugOutput)
            {
                Debug.Log($"Object #{objectNumber} too small with {mesh.vertexCount} verts - not creating GameObject.");
            }
            return null;
        }
        ...
        AssignUVs(mesh);
        go.transform.position += transform.TransformVector(centroid);
        mf.mesh = mesh;

//        Debug.Log($"{stopwatch.ElapsedMilliseconds} ms - object ready.");
        return go;
    }

    // Add a xz point to the list of points in a contour, but don't create duplicates if the point already exists.
    private void AddToList(List<Vector2Int> list, Vector2Int point)
    {
    ...
    /// So we iterate through rows (z-coordinates) in the array and get the leftmost and rightmost point of each row.
    ...
    public List<Vector2Int> TraceContour(short[,] array, int objectNumber)
    {
        ...
        HashSet<Vector2Int> uniqueContourPoints = new HashSet<Vector2Int>(); // Use a hashSet to keep track of unique points and avoid adding duplicates to the list.
        ...
        for (int z = 0; z < array.GetLength(1); z++)
        {
            ...
            // it means that this shape has completed and no results will be found in following rows.
    ...
    /// Takes a list of contour points of a 2D plane in clockwise order and fan triangulates a mesh out of it.
    public Mesh CreateHorizontalMeshFromContourPoints(List<Vector2Int> contourPoints, int height, bool reverse)
    {
        ...
        // Step 3: Compute the centroid of the polygon
        ...
        {
//            Debug.LogWarning("Central points list for this object is empty. Adding centroid as central point.");
        ...
        for (int i = 0; i < contourPointsXZ.Count; i++)
        {
            ...
            {
                //// line to map based on overall distance
                //float dist = Vector2.Distance(new Vector2(contourPoint.x, contourPoint.z), new Vector2(centralPoints[j].x, centralPoints[j].z));

                // currently mapping based on z-distance only - better for triangulation
        ...
        for (int i = 0; i < contourPointsXZ.Count; i++)
        {
            ...
            {
                // Both contour points share the same central point
            ...
            {
                // The contour points are connected to different central points
                // Create quad (two triangles) between the central points and contour points
                if (!reverse)
                {
                    // Triangle 1
                    triangles.Add(centralIndex1);
    ...
    /// For straight lines along the z axis in the object's contour, optimise the list of points by preserving only the start and end points
    /// Except if the line is too long (e.g. more than 20 points) then create intermediate points to help with mesh triangulation.
